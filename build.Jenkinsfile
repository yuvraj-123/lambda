...
def deploylambda(dir) {
    ...
    sh "cd ${dir}/ && sam deploy --no-confirm-changeset --no-fail-on-empty-changeset --template-file template.yaml --config-file ${ENVIRONMENT}-config.toml --config-env ${ENVIRONMENT} --s3-bucket ${s3_bucket} --parameter-overrides LambdaFunctionName=${ENVIRONMENT}-${dir}" 
    }
