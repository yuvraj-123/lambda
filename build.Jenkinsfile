...
def deploylambda(dir) {
    ...
    sh "cd ${dir}/ && sam deploy --no-fail-on-empty-changeset --template-file packaged-template.yml --config-file config.toml --config-env ${ENVIRONMENT} --s3-bucket ${s3_bucket}"
    }
